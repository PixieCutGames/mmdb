...
type SearchResultsProps = MMDBProps<{
  phrase: string;
}>;
export default function SearchResults({ phrase }: SearchResultsProps) {
  const [_, setSearchPhrase] = useState("");
  ...
  return (
    <ul className="absolute w-full bg-main mt-1.5 rounded border border-main-light overflow-hidden z-10">
      ...
      <li>
        <Link
          className="flex p-4 justify-between items-center bg-main-dark hover:bg-success group"
          href={"/search/" + phrase}
          onClick={onClick}
        >
