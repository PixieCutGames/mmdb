...
export default function MovieList({ title, list, loading }: MovieListProps) {
    ...
    <div className="xl:max-w-screen-xl lg:max-w-screen-lg w-full mb-4">
      ...
      <main className="bg-main-dark overflow-x-auto whitespace-nowrap">
        {list.map((movie) => (
          <div
            key={movie.id}
            className="inline-block w-6/12 md:w-4/12 lg:w-3/12 mx-2 my-3 relative group cursor-pointer"
          >
            <Poster
            ...
            <div className="absolute w-full h-full top-0 left-0 overflow-hidden">
              ...
              <div className="transition-bottom ease-in-out duration-500 flex bg-main-dark absolute -bottom-11 w-full group-hover:bottom-0">
                {!authenticated ? (
                  <span className="hidden group-hover:inline-block whitespace-normal text-center">
                    <Tooltip
                      width="200px"
                ...
                <button
                  title="own it"
                  className={`group relative flex-1 flex justify-center items-center py-3 border-t-4 ${
                    authenticated
